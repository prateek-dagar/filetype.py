...
def test_infer_image_from_bytes(benchmark):
    benchmark(filetype.guess, bytearray([0xFF, 0xD8, 0xFF, 0x00, 0x08]))


def test_infer_video_from_bytes(benchmark):
    benchmark(filetype.guess, bytearray([0x1A, 0x45, 0xDF, 0xA3, 0x08]))


def test_infer_audio_from_bytes(benchmark):
    benchmark(filetype.guess, bytearray([0x4D, 0x54, 0x68, 0xA3, 0x64]))
